...
{{#ifne Operations.Count 0}}
  ...
  {{#if DataExtensionsRequired}}
    ...
    {{#each DataOperations}}
      {{#ifeq Type 'GetColl'}}
        ...
        private Func<IQueryable<{{Parent.EntityFrameworkEntity}}>, {{#each CoreDataParameters}}{{{ParameterType}}}, {{/each}}{{DataArgs.Type}}, IQueryable<{{Parent.EntityFrameworkEntity}}>>? {{PrivateName}}OnQuery;
        {{/ifeq}}   
        {{#ifeq AutoImplement 'Cosmos'}}
          {{#ifval Parent.CosmosValueContainer}}
        private Func<IQueryable<CosmosDbValue<{{Parent.CosmosEntity}}>>, {{#each CoreDataParameters}}{{{ParameterType}}}, {{/each}}{{DataArgs.Type}}, IQueryable<CosmosDbValue{{Parent.CosmosEntity}}>>>? {{PrivateName}}OnQuery;
          {{else}}
        private Func<IQueryable<{{Parent.CosmosEntity}}>, {{#each CoreDataParameters}}{{{ParameterType}}}, {{/each}}{{DataArgs.Type}}, IQueryable<{{Parent.CosmosEntity}}>>? {{PrivateName}}OnQuery;
          {{/ifval}}
        {{/ifeq}}  
        {{#ifeq AutoImplement 'OData'}}
        private Func<Soc.IBoundClient<{{Parent.ODataEntity}}>, {{#each CoreDataParameters}}{{{ParameterType}}}, {{/each}}{{DataArgs.Type}}, Soc.IBoundClient<{{Parent.ODataEntity}}>>? {{PrivateName}}OnQuery;
        {{/ifeq}} 
      {{/ifeq}}
  ...
  {{#each DataOperations}}
    ...
    {{#ifeq AutoImplement 'None'}}
      {{#if Parent.DataExtensions}}
            => DataInvoker.Current.InvokeAsync(this, () => {{Name}}OnImplementationAsync({{#ifeq Type 'Create' 'Update'}}Check.NotNull(value, nameof(value)){{/ifeq}}{{#each ValueLessDataParameters}}{{#unless @first}}, {{/unless}}{{ArgumentName}}{{/each}}), new BusinessInvokerArgs { ExceptionHandler = {{PrivateName}}OnException{{#if DataTransaction}}, IncludeTransactionScope = true{{/if}} });
      {{else}}
            => DataInvoker.Current.InvokeAsync(this, () => {{Name}}OnImplementationAsync({{#ifeq Type 'Create' 'Update'}}Check.NotNull(value, nameof(value)){{/ifeq}}{{#each ValueLessDataParameters}}{{#unless @first}}, {{/unless}}{{ArgumentName}}{{/each}}){{#if DataTransaction}}, new BusinessInvokerArgs { IncludeTransactionScope = true }{{/if}});
      {{/if}}
    {{else}}
        {
            ...
            {
      {{#ifeq Type 'GetColl'}}
                {{OperationReturnType}} __result = new {{OperationReturnType}}({{#if PagingArgs}}paging{{/if}});
      {{else}}
        {{#if Parent.DataExtensions}}
          {{#if HasReturnValue}}
      ...
      {{/ifeq}}
      {{#ifeq AutoImplement 'Database'}}
                var __dataArgs = {{DataEntityMapper}}.Default.CreateArgs("[{{Parent.DatabaseSchema}}].[{{DatabaseStoredProc}}]"{{#if PagingArgs}}, __result.Paging!{{/if}});
      {{/ifeq}}
      {{#ifeq AutoImplement 'EntityFramework'}}
                var __dataArgs = {{DataEntityMapper}}.Default.CreateArgs({{#if PagingArgs}}__result.Paging!{{/if}});
      {{/ifeq}}
      {{#ifeq AutoImplement 'Cosmos'}}
                var __dataArgs = {{DataEntityMapper}}.Default.CreateArgs("{{Parent.CosmosContainerId}}"{{#if PagingArgs}}, __result.Paging!{{/if}}, {{CosmosPartitionKeyCode}}, onCreate: _onDataArgsCreate);
      {{/ifeq}}
      {{#ifeq AutoImplement 'OData'}}
                var __dataArgs = {{DataEntityMapper}}.Default.CreateArgs({{#if PagingArgs}}__result.Paging!{{/if}}{{#ifval ODataCollectionName}}{{#if PagingArgs}}, {{/if}}"{{ODataCollectionName}}"{{/ifval}});
      {{/ifeq}}
      {{#if Parent.DataExtensions}}
      ...
      {{#ifeq AutoImplement 'EntityFramework'}}
        {{#ifeq Type 'GetColl'}}
                __result.Result = {{DataArgs.Name}}.Query(__dataArgs, q => {{PrivateName}}OnQuery?.Invoke(q, {{#each CoreDataParameters}}{{{ArgumentName}}}, {{/each}}__dataArgs) ?? q).SelectQuery<{{Parent.EntityCollectionName}}>();
        {{else}}
                {{#if Parent.DataExtensions}}{{#if HasReturnValue}}__result = {{/if}}{{else}}{{#if HasReturnValue}}return {{/if}}{{/if}}await {{DataArgs.Name}}.{{Type}}Async(__dataArgs{{#each PagingLessDataParameters}}, {{#if IsValueArg}}Check.NotNull(value, nameof(value)){{else}}{{ParameterConverted}}{{/if}}{{/each}}).ConfigureAwait(false);
        {{/ifeq}}
      {{/ifeq}}
      {{#ifeq AutoImplement 'Cosmos'}}
        {{#ifeq Type 'GetColl'}}
                __result.Result = {{DataArgs.Name}}.{{#if CosmosValueContainer}}Value{{/if}}Container(__dataArgs).Query(q => {{PrivateName}}OnQuery?.Invoke(q, {{#each CoreDataParameters}}{{{ArgumentName}}}, {{/each}}__dataArgs) ?? q).SelectQuery<{{Parent.EntityCollectionName}}>();
        {{else}}
...
{{#if UsesEntityFramework}}
  {{#unless EntityFrameworkCustomMapper}}
        ...
        /// <summary>
        /// Provides the {{{EntityNameSeeComments}}} and Entity Framework {{{seecomments EntityFrameworkEntity}}} property mapping.
        /// </summary>
        [System.Diagnostics.CodeAnalysis.SuppressMessage("Design", "CA1034:Nested types should not be visible", Justification = "By design; as there is a direct relationship")]
        public partial class EfMapper : EfDbMapper<{{EntityName}}, {{EntityFrameworkEntity}}, EfMapper>
        {
            /// <summary>
...
{{#if UsesCosmos}}
  {{#unless CosmosCustomMapper}}
        ...
        /// <summary>
        /// Provides the {{{EntityNameSeeComments}}} and Cosmos {{{seecomments EntityFrameworkEntity}}} property mapping.
        /// </summary>
        [System.Diagnostics.CodeAnalysis.SuppressMessage("Design", "CA1034:Nested types should not be visible", Justification = "By design; as there is a direct relationship")]
        public partial class CosmosMapper : CosmosDbMapper<{{EntityName}}, {{CosmosEntity}}, CosmosMapper>
        {
            /// <summary>
...
{{#if UsesOData}}
  {{#unless ODataCustomMapper}}
        ...
        /// <summary>
        /// Provides the {{{EntityNameSeeComments}}} and OData {{{seecomments EntityFrameworkEntity}}} property mapping.
        /// </summary>
        [System.Diagnostics.CodeAnalysis.SuppressMessage("Design", "CA1034:Nested types should not be visible", Justification = "By design; as there is a direct relationship")]
        public partial class ODataMapper : ODataMapper<{{EntityName}}, {{ODataEntity}}, ODataMapper>
        {
            /// <summary>
