...
using {{Root.NamespaceCdcPublisher}}.Entities;
...
{{#each CdcJoins}}
  ...
  {{#each JoinHierarchyReverse}}
                   ...
                   {{indent IndentSpaces}} {
    {{#unless @last}}
                   {{indent IndentSpaces}}     var {{#ifval HierarchyChild}}{{HierarchyChild.Alias}}{{else}}{{Parent.Alias}}{{/ifval}}Item = {{#ifval HierarchyChild}}{{HierarchyChild.Alias}}{{else}}{{Parent.Alias}}{{/ifval}}Coll.Single(x => {{#each OnSelectColumns}}{{#unless @last}} && {{/unless}}x.{{ToColumnAlias}} == {{Parent.Alias}}.{{#if @../last}}{{ToColumnAlias}}{{else}}{{pascal Parent.JoinTo}}_{{NameAlias}}{{/if}}{{/each}}).{{PropertyName}};
    {{else}}
