...
{{#each DataSvcOperations}}
  ...
  {{else}}
    {{#if SupportsCaching}}
      ...
      {{/ifeq}}
    {{/if}}         
                {{#if HasReturnValue}}var __result = {{/if}}await _data.{{Name}}Async({{#ifeq Type 'Create' 'Update'}}Check.NotNull(value, nameof(value)){{/ifeq}}{{#each ValueLessDataParameters}}{{#unless @first}}, {{/unless}}{{{ArgumentName}}}{{/each}}).ConfigureAwait(false);
    {{#ifeq Events.Count 1}}
      {{#each Events}}
