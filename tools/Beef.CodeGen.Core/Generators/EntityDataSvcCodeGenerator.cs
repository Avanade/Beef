...
namespace Beef.CodeGen.Generators
{
    ...
    /// </summary>
    public class EntityDataSvcCodeGenerator : CodeGeneratorBase<CodeGenConfig, EntityConfig>
    {
        /// <summary>
        /// <inheritdoc/>
        /// </summary>
        ...
        protected override IEnumerable<EntityConfig> SelectGenConfig(CodeGenConfig config)
            => (config ?? throw new System.ArgumentNullException(nameof(config))).Entities!.Where(x => IsFalse(x.ExcludeDataSvc) && x.Operations!.Count > 0).AsEnumerable();
    }
}