...
`stringCasing` | The `string` casing to be performed on `Set` and `CleanUp`. Valid options are: `UseDefault`, `None`, `Lower`, `Upper`, `Title`.<br/>&dagger; Defaults to `UseDefault`. This is only applied where the `Type` is `string`.
