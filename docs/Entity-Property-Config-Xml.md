...
`IgnoreSerialization` | Indicates whether the property is not to be serialized.<br/><br/>All properties are serialized by default.
...
Property | Description
-|-
`DataDatabaseMapper` | The database property `Mapper` class name where `Entity.AutoImplement` is selected.<br/><br/>A `Mapper` is used to map a data source value to/from a .NET complex `Type` (i.e. class with one or more properties).
