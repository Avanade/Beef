...
`text` | The overriding text for use in comments.<br/><br/>By default the `Text` will be the `Name` reformatted as sentence casing.
...
Property | Description
-|-
`refDataType` | The underlying Reference Data Type that is also used as the Reference Data serialization identifier (SID). Valid options are: `string`, `int`, `Guid`.<br/><br/>Defaults to `string` where not specified and the corresponding `Type` starts with (prefix) `RefDataNamespace.`.
...
`iValidator` | The name of the .NET Interface that the `Validator` implements/inherits.<br/><br/>Defaults to `IValidator<{Type}>` where the `{Type}` is `Type`.
...
Property | Description
-|-
`dataConverter` | The data `Converter` class name where specific data conversion is required.<br/><br/>A `Converter` is used to convert a data source value to/from a .NET `Type` where the standard data type conversion is not applicable.
