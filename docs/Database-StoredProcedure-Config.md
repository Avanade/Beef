# 'StoredProcedure' object (database-driven)
...
**`type`** | The stored procedure operation type. Valid options are: `Get`, `GetColl`, `Create`, `Update`, `Upsert`, `Delete`, `Merge`.<br/><br/>Defaults to `GetColl`.
...
Property | Description
-|-
`mergeOverrideIdentityColumns` | The list of `Column` names to be used in the `Merge` statement to determine whether to _insert_, _update_ or _delete_.<br/><br/>This is used to override the default behaviour of using the primary key column(s).
