...
{
    /// <summary>
    /// <see cref="SendEventsAsync(EventData[])">Send</see> the <see cref="EventData"/> array (converted to <see cref="AzureEventHubs.EventData"/>) using the same partition key (see <see cref="GetPartitionKey(EventData[])"/>.
    /// </summary>
    public class EventHubPublisher : EventPublisherBase
    {
        private readonly AzureEventHubs.EventHubClient _client;
        private readonly EventHubPublisherInvoker _invoker;

        /// <summary>
        /// Initializes a new instance of the <see cref="EventHubPublisher"/> using the specified <see cref="AzureEventHubs.EventHubClient"/> (consider setting the underlying
        /// <see cref="AzureEventHubs.ClientEntity.RetryPolicy"/>) to allow for transient errors).
        /// </summary>
        /// <param name="client">The <see cref="AzureEventHubs.EventHubClient"/>.</param>
        /// <param name="invoker">Enables the <see cref="Invoker"/> to be overridden; defaults to <see cref="EventHubPublisherInvoker"/>.</param>
        public EventHubPublisher(AzureEventHubs.EventHubClient client, EventHubPublisherInvoker? invoker = null)
        {
        ...
        protected override async Task SendEventsAsync(params EventData[] events)
        {
            ...
            var eventHubEvents = new AzureEventHubs.EventData[events.Length];
        ...
        /// <summary>
        /// Invoked when the underlying <see cref="AzureEventHubs.EventHubClient.SendAsync(EventHubs.EventData, string)"/> results in an unhandled <see cref="Exception"/>;
