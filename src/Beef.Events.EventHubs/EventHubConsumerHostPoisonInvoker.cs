...
{
    /// <summary>
    /// Represents an <see cref="InvokerBase{EventHubData}">invoker</see> that uses an <see cref="IEventHubStorageRepository"/> to manage poison events (both retry and skipping).
    /// </summary>
    public class EventHubConsumerHostPoisonInvoker : InvokerBase<EventHubData, Result>
    {
        /// <summary>
        ...
        protected async override Task<Result> WrapInvokeAsync(object caller, Func<Task<Result>> func, EventHubData? data, [CallerMemberName] string? memberName = null, [CallerFilePath] string? filePath = null, [CallerLineNumber] int lineNumber = 0)
        {
            if (func == null)
