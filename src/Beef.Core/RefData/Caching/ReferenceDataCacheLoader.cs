...
namespace Beef.RefData.Caching
{
    ...
    public class ReferenceDataCacheLoader
    {
            ...
            where TItem : ReferenceDataBase, new()
        {
            ...
            sw.Stop();
            Logger.Create<ReferenceDataCacheLoader>().LogDebug("ReferenceData Cache Type '{0}' loaded with {1} items on thread '{2}' in {3}ms.", typeof(TItem).Name, coll.Count, Thread.CurrentThread.ManagedThreadId, sw.ElapsedMilliseconds);
            return coll;
        }
    }
}