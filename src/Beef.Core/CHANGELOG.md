...
## v4.1.3
- *Enhancement:* **Breaking change** to _Validation_ to enable async. The existing methods `Validate` and `Run` have been changed to be `ValidateAsync` and `RunAsync` respectively with a response of `Task`. A number of the `Rules` have been updated to support async overloads.
- *Enhancement:* **Breaking change** to _Validation_ to enable Dependency Injection (DI), removed `Validator<TEntity, TValidator>` to eliminate usage of existing static `Default` property.
