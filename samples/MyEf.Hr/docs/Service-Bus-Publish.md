...
The latest `CoreEx.Azure` NuGet [package](https://www.nuget.org/packages/CoreEx.Azure) will need to be added to the `MyEf.Hr.Api` project. Additionally, the `Startup` will require the following `using` statements added.
...
The [`HrSettings`](../MyEf.Hr.Business/HrSettings.cs) class should have the following properies added.
...
`ServiceBusSender:QueueOrTopicName`* | The Azure Service Bus Queue or Topic name depending on configuration (requirements depending).
