CREATE PROCEDURE [XCdc].[spCreateCdcIdentifierMapping]
  @IdentifierList AS [XCdc].[udtCdcIdentifierMappingList] READONLY
AS
BEGIN
  /*
  ...
  BEGIN TRY
    ...
    INSERT INTO [XCdc].[CdcIdentifierMapping]
        ([Schema], [Table], [Key], [GlobalId])
      SELECT [n].[Schema], [n].[Table], [n].[Key], [n].[GlobalId]
        FROM @IdentifierList AS [n]
        WHERE NOT EXISTS (SELECT 0 FROM [XCdc].[CdcIdentifierMapping] AS [o]
    ...
    SELECT [n].[Schema], [n].[Table], [n].[Key], [n].[GlobalId]
      FROM [XCdc].[CdcIdentifierMapping] AS [n]
