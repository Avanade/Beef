CREATE PROCEDURE [DemoCdc].[spCompleteContactCdcOutbox]
  @OutboxId INT,
...
BEGIN
  ...
  BEGIN TRY
    ...
    MERGE INTO [DemoCdc].[CdcTracking] WITH (HOLDLOCK) AS [_ct]
      USING @TrackingList AS [_list] ON ([_ct].[Schema] = 'Legacy' AND [_ct].[Table] = 'Contact' AND [_ct].[Key] = [_list].[Key])
      ...
      WHEN NOT MATCHED BY TARGET
        THEN INSERT ([Schema], [Table], [Key], [Hash], [OutboxId])
          VALUES ('Legacy', 'Contact', [_list].[Key], [_list].[Hash], @OutboxId);
