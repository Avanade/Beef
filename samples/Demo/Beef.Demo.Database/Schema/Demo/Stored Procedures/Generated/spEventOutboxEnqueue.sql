CREATE PROCEDURE [Demo].[spEventOutboxEnqueue]
  @EventList AS [Demo].[udtEventOutboxList] READONLY
AS
BEGIN
  /*
  ...
  BEGIN TRY
    ...
    DECLARE c CURSOR FORWARD_ONLY 
      FOR SELECT [EventId], [Subject], [Action], [CorrelationId], [TenantId], [ValueType], [EventData] FROM @EventList

    OPEN c
    FETCH NEXT FROM c INTO @eventId, @subject, @action, @correlationId, @tenantId, @valueType, @eventdata

    -- Iterate the event(s).
    WHILE @@FETCH_STATUS = 0
    BEGIN
        -- Enqueue event into outbox 
        INSERT INTO [Demo].[EventOutbox] ([EnqueuedDate])
        ...
        INSERT INTO [Demo].[EventOutboxData] (
          [EventOutboxId],
          [EventId],
          [Subject], 
          [Action], 
