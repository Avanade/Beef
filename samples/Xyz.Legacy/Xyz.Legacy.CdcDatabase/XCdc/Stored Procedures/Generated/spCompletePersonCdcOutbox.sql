CREATE PROCEDURE [XCdc].[spCompletePersonCdcOutbox]
  @OutboxId INT,
  @TrackingList AS [XCdc].[udtCdcTrackingList] READONLY
AS
BEGIN
  /*
  ...
  BEGIN TRY
    ...
    MERGE INTO [XCdc].[CdcTracking] WITH (HOLDLOCK) AS [_ct]
      USING @TrackingList AS [_list] ON ([_ct].[Schema] = 'Legacy' AND [_ct].[Table] = 'Person' AND [_ct].[Key] = [_list].[Key])
      ...
      WHEN NOT MATCHED BY TARGET
        THEN INSERT ([Schema], [Table], [Key], [Hash], [OutboxId])
          VALUES ('Legacy', 'Person', [_list].[Key], [_list].[Hash], @OutboxId);
